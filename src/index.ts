/* ============================================================
   ...
   Cloudflare Workers – Inline PM Configuration Version
   ============================================================ */
...
export interface Env {
  ...
  OWNER_USER_IDS: string; // user ids + group ids
...
export default {
  async fetch(req: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (update.callback_query) {
      ...
      ctx.waitUntil(routeMessage(update.message, env));
...
function allowedSet(env: Env): Set<string> {
  return new Set(
...
async function handlePrivate(msg: TgMessage, env: Env) {
  if (!msg.from || !allowedSet(env).has(String(msg.from.id))) {
...
async function handleCallback(q: TgCallbackQuery, env: Env) {
  ...
  if (!allowedSet(env).has(String(q.from.id))) {
    await answerCb(q.id, "Not allowed", env);
    return;
  }

  if (data === "help") {
    await answerCb(q.id, "Use buttons below", env);
    return;
  }
  ...
  if (data.startsWith("group:")) {
    ...
    await showGroupSettings(chatId, gid, env);
    await answerCb(q.id, "", env);
    return;
  }
  ...
  if (data.startsWith("toggle:")) {
    ...
    await showGroupSettings(chatId, Number(gid), env);
    await answerCb(q.id, "", env);
    return;
  }
...
async function showGroups(chatId: number | string, env: Env) {
  ...
  await sendInline(chatId, "📋 Select a group:", buttons, env);
}
...
async function showGroupSettings(
  ...
  await sendInline(
    ...
    [
      ...
      [
        { text: `➖ Warn`, data: `warn:${groupId}:down` },
        { text: `Warn: ${s.warnLimit}`, data: "noop" },
        { text: `➕`, data: `warn:${groupId}:up` }
      ],
      [
        { text: `➖ Mute`, data: `mute:${groupId}:down` },
...
async function handleGroup(msg: TgMessage, env: Env) {
  ...
  const settings = await getSettings(msg.chat.id, env);
  await rememberGroup(msg.chat, env);
  ...
  if (settings.antiforward && (msg.story || msg.forward_origin || msg.is_automatic_forward)) {
    await violation(msg.chat.id, msg.from.id, msg.message_id, "Forwarded content", env);
    return;
  }

  if (settings.antilink && hasLink(text, settings.whitelist)) {
    await violation(msg.chat.id, msg.from.id, msg.message_id, "Links not allowed", env);
  }
}
...
async function violation(
  ...
  if (count >= s.warnLimit) {
    ...
    await sendTemp(chatId, `🔇 Auto-muted\nReason: ${reason}`, 300, env);
  } else {
...
async function rememberGroup(chat: TgChat, env: Env) {
  await env.BOT_CONFIG.put(`group:${chat.id}`, JSON.stringify({
    id: chat.id,
    title: chat.title || ""
  }));
}
...
async function runDeletionCron(env: Env) {
  ...
  for (const k of list.keys) {
    ...
    if (v.at <= now) {
      ...
      if (v.also) await del(v.chatId, v.also, env);
